...
function addLevel(){
	 ...
	 $.ajax({
			...
			success : function(response) {
				...
				$("#levels").html('<span id="msg" style="color:red;">'+response.msg+'</span>');
				$("#testName")[0].selectedIndex = 0;
				getTest();
			 ...
			 success:function(response){
 			 ...
 			 for (var i =0; i< response.levels.length ; i++){
				console.log(response.levels[i].testName);
					$("#levels").append(
							"<table style='width:100%;'><tr class='tr1'><td><label class='fieldtitle'><b>Level"+(i+1)+"</b></label></td><td id="+ response.levels[i].id+ ">"
							+ response.levels[i].testName+ 
							"&nbsp;&nbsp; <a href='#' id='edit' onclick='javascript:editLevel("+ response.levels[i].id+ ");'><span class='glyphicon glyphicon-edit'></span>Edit</a></td></tr></table>")
	 		}
