...
<body id="top" class="has-header-search" onload="setTimeOnLoad()">
					...
					<div class="promo-btn">
						<div class="durationinfo" style="font-size: 20px;">
							<span><i class="fa fa-clock-o" style="font-size: 25px;"></i>
								<span class="time" id="timer"><i id="hours"></i><sub>h</sub><i
			...
			currentQuestion.questionMapperInstance.questionMapper.question.choice6.trim().length() == 0? 'display: none;':'clear:left;font-size: 14px;'}">
					...
					<c:choose>
						<c:when test="${currentSection.last==true}">
							<a href="javascript:showAttemptModal();"
								class="next waves-effect waves-light btn submit-button mt-30">FINISH</a>
	...
	<div id="attemptModel" class="modal fade" role="dialog">
		<div class="modal-dialog">
			<!-- Modal content-->
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Attempt Information</h4>
				</div>
				<div class="modal-body">
					<table class="table" id="attemptbl">
						<thead>
							<tr>
								<th>Section</th>
								<th>Not Attempted</th>
				...
				<div class="modal-footer">
					<button type="button"
						class="close,next waves-effect waves-light btn submit-button mt-30"
...
//edt.value = editor.getSession().getValue();
	var linktext=document.getElementById('next').text;
		...
		//alert(document.testForm.action );
		storeTimeLocal();
	...
	function showAttemptModal(){
		$.ajax({
			type : 'GET',
			url : 'beforeSubmitTest?questionId=${currentQuestion.questionMapperInstance.questionMapper.id}',
			contentType: "application/json; charset=utf-8",
			success : function(data) {
				 $('#attemptbl tbody').find("tr").remove();
				var row="";
				for(var i=0;i<data.length;i++){
					row += "<tr><td>"+data[i]['section']+"</td><td>"+data[i]['notAttempt']+"</td><td>"+data[i]['setNoOffQuestion']+"</td></tr>";
				}
				 $('#attemptbl tbody').append(row);
				 $('#attemptModel').modal('show');
			},
			error : function(e) {
				console.log("ERROR: ", e);
